...
private:
  ...
  void initImgui() {
    ...
    // Optional: Set render pass if your version supports it separately
    // ImGui_ImplVulkan_SetRenderPass(renderPass);
  }
  ...
  void pickPhysicalDevice() {
    ...
    if (deviceCount == 0) {
      throw std::runtime_error("[x] Failed to find GPUs with Vulkan support!");
    }
  ...
  void createInstance() {
    if (enableValidationLayers && !checkValidationLayerSupport()) {
      throw std::runtime_error(
          "[x] Validation layers requested but not available");
    }
  ...
  bool checkValidationLayerSupport() {
    ...
    for (const char *layerName : validationLayers) {
      ...
      if (!layerFound) {
        throw std::runtime_error("[x] Failed to load layer " +
  ...
  void drawFrame() {
    ...
    // Required cause we want to draw only one buffer at a time
